...
public class RegionRemoteControl extends View {
    ...
    int mTouchedColor = Color.DKGRAY;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //根据视图大小，初始化Path 和 Region
        center_p.addCircle(0,0,0.2f*minWidth, Path.Direction.CCW);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (touchFlag==CENTER){
            canvas.drawPath(center_p,mDefaultPaint);
        }else if (touchFlag==RIGHT){
            canvas.drawPath(right_p,mDefaultPaint);
        }else if (touchFlag==UP){
            canvas.drawPath(up_p,mDefaultPaint);
        }else if (touchFlag==DOWN){
            canvas.drawPath(down_p,mDefaultPaint);
